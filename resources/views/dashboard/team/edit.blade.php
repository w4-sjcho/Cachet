    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-sm-12">
                ...
                <form name="UserForm" class="form-vertical" role="form" action="{{ route('dashboard.team.edit', [$user->id]) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        @if($current_user->isAdmin)
                        <a class="btn btn-info" href="{{ route('dashboard.user.api.regen', [$user->id]) }}">{{ trans('cachet.api.revoke') }}</a>
                        @if($current_user->id != $user->id)
                        <a class="btn btn-danger confirm-action" href="{{ route('dashboard.team.delete', [$user->id]) }}" data-method="DELETE">{{ trans('forms.delete') }}</a>
                        @endif
                        @endif
                    </div>
                </form>
